...
class Html(object):
    ...
    def añadirEtiquetaVideo(self,src,title, modo = None):
        atributos = {'src': src, 'title':title}
        if modo:
            atributos[modo] = modo
...
def main():
    ...
    for video in videos:
        title = video.get("descripcion")
        nuevoHtml.añadirEtiquetaVideo(video.text,title,'controls')  
